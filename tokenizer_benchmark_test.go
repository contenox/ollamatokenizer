...
func createBenchTokenizer(b *testing.B, model string) ollamatokenizer.Tokenizer {
	...
	tokenizer, err := ollamatokenizer.NewTokenizer(
		...
		ollamatokenizer.TokenizerWithPreloadedModels(model),
		ollamatokenizer.TokenizerWithFallbackModel(model),
	)
	if err != nil {
